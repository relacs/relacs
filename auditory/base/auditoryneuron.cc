...
void AuditoryNeuron::main( void )
{
  ...
  int nonlin = index( "nl" );
  if ( nonlin == 0 )
    Nonlinearity = &AuditoryNeuron::square;
  else if ( nonlin == 1 )
    Nonlinearity = &AuditoryNeuron::squareSaturated;
  else if ( nonlin == 2 )
    Nonlinearity = &AuditoryNeuron::linearBoltzman;
  else if ( nonlin == 3 )
    Nonlinearity = &AuditoryNeuron::squareBoltzman;
  else if ( nonlin == 4 )
    Nonlinearity = &AuditoryNeuron::linear;
  else if ( nonlin == 5 )
    Nonlinearity = &AuditoryNeuron::linearSaturated;
  else if ( nonlin == 6 )
    Nonlinearity = &AuditoryNeuron::box;
  else
    Nonlinearity = &AuditoryNeuron::identity;
  Imax = number( "imax" );
  Imin = number( "imin" );
  CutPoint = number( "cut" );
  DI = Imax-Imin;
...
void AuditoryNeuron::process( const OutData &source, OutData &dest )
{
...
double AuditoryNeuron::identity( double x ) const
{
  return DIC*x;
}


double AuditoryNeuron::box( double x ) const
{
  return fabs(x) < CutPoint ? Imin : Imax;
}


double AuditoryNeuron::linear( double x ) const
{
  return DIC*fabs(x) + Imin;
}


double AuditoryNeuron::linearSaturated( double x ) const
{
...
double AuditoryNeuron::square( double x ) const
{
  return DICC*x*x + Imin;
}


double AuditoryNeuron::squareSaturated( double x ) const
{
...
double AuditoryNeuron::linearBoltzman( double x ) const
{
  /*
