...

{
  ...
  SessionButton = new QPushButton( "Cell Found", bb, "AuditorySessionButton" );
...
void AuditorySession::initialize( void )
{
  // get trace indices:
  EPhysTraces::initialize( traces(), events() );
  AuditoryTraces::initialize( this, traces(), events() );
...
void AuditorySession::initDevices( void )
{
...
void AuditorySession::startSession( void )
{
...
void AuditorySession::stopSession( bool saved )
{
  lock();
...
void AuditorySession::main( void )
{
  if ( Temp == 0 )
    return;
...
void AuditorySession::startResistance( void )
{
...
void AuditorySession::measureResistance( void )
{
  if ( Ampl != 0 && SpikeTrace[0] >= 0 && RMeasure ) {
    readLockData();
...
void AuditorySession::stopResistance( void )
{
  if ( Ampl != 0 && SpikeTrace[0] >= 0 && RMeasure ) {
    Ampl->manual();
    readLockData();
...
void AuditorySession::buzz( void )
{
  if ( Ampl != 0 ) {
    Ampl->buzzer( );
  }
...
void AuditorySession::keyPressEvent( QKeyEvent *e )
{
...
void AuditorySession::keyReleaseEvent( QKeyEvent *e )
{
  switch ( e->key() ) {
...
void AuditorySession::notifyMetaData( void )
{
...
MapD AuditorySession::threshCurve( int side ) const
{
  if ( side > 1 ) {
    lockMetaData();
...
void AuditorySession::addThreshCurve( const MapD &thresh, int side )
{
  lock();
...
MapD AuditorySession::fICurve( int side ) const
{
  if ( side > 1 ) {
    lockMetaData();
    side = metaData().index( "best side" );
...
void AuditorySession::addFICurve( const MapD &ficurve, int side )
{
  lock();
...
MapD AuditorySession::onFICurve( int side ) const
{
  if ( side > 1 ) {
    lockMetaData();
    side = metaData().index( "best side" );
...
void AuditorySession::addOnFICurve( const MapD &onficurve, int side )
{
  lock();
...
MapD AuditorySession::ssFICurve( int side ) const
{
  if ( side > 1 ) {
    lockMetaData();
    side = metaData().index( "best side" );
...
void AuditorySession::addSSFICurve( const MapD &ssficurve, int side )
{
  lock();
...
void AuditorySession::updateBestSide( void )
{
  lockMetaData();
  int bestnoise = -1;
...
void AuditorySession::plot( void )
{
  lock();
  lockMetaData();
  P.lock();
