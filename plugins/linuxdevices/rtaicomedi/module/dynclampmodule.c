...
int syncSECPulse = 0;  /*! Length of the inject pulse generated by the SEC in ns. If 0 disable generation of pulses. */
int syncSECMode = 0;  /* 0: fixed period, 1: difftime, n>1: average over n difftime */
...
int setDigitalIO( struct dioIOCT *dioIOC )
{
  ...
  if ( dioIOC->op == DIO_CONFIGURE ) {
    ...
    if ( retval != 1 ) {
      ...
      ERROR_MSG( "setDigitalIO: comedi_dio_write on device %s subdevice %u failed! returned %d\n",
		 devname, subdevice, retval );
      ERROR_MSG( "setDigitalIO: disabled syncSECPulse.\n" );
      return -EFAULT;
    }
    else {
      ...
      DEBUG_MSG( "setDigitalIO: comedi_dio_write for setting sync pulse high on device %s subdevice %u succeeded. New pulse is %d us\n",
		 devname, subdevice, syncSECPulse );
    }
#else
    return -ENOTTY;
#endif
    ...
    if ( syncSECPulse > 0 ) {
      ...
      if ( retval != 1 ) {
	comedi_perror( "setDigitalIO() -> DIO_CLEAR_SYNCPULSE" );
	ERROR_MSG( "setDigitalIO: comedi_dio_write for setting sync pulse low on device %s subdevice %u failed! returned %d\n",
		   devname, subdevice, retval );
	return -EFAULT;
      }
    }
...
void dynclamp_loop( long dummy )
{
  ...
  while( aisubdev.running > 0 ) {
	...
	*/
	...
	ERROR_MSG( "dynclamp_loop: ERROR! failed to set DIO low at start write for syncing SEC with retunr value %d\n", retVal );
      }
    }
#endif
