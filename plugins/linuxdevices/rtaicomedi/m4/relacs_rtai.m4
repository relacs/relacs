...
# set RTAI installation directory:
AS_IF( [ test "x$with_rtai" = "xdetect" ],
       [ AS_IF( [ test -d /usr/realtime/`uname -r` ], 
       	        [ RTAI_PATH="/usr/realtime/`uname -r`"
		  USE_RTAI=yes; ],
...
AS_IF( [ test $USE_RTAI = yes ],
	 ...
	 AS_IF( [ test "x$RTAI_CPPFLAGS" = x ], 
	 	[ RTAI_CPPFLAGS="-I /usr/include -I /usr/realtime/include -ffast-math -mhard-float" ] ) 
       ] )
...
AS_IF( [ test $USE_RTAI = yes ],
	 ...
	 AC_MSG_NOTICE( "RTAI_CPPFLAGS=${RTAI_CPPFLAGS}" )
       ] )
...
AS_IF( [ test $USE_RTAI = yes ],
       [ AC_CHECK_HEADERS( [rtai.h],
       	 		   [ AS_IF( [ test "x${with_rtai}" != xdetect ],
        		     	    [ RELACS_RTAI=$RTAI_PATH ],
        			    [ RELACS_RTAI="yes" ] ) ], 
     			   [ AC_MSG_NOTICE( "RTAI include files not found in path ${RTAI_PATH}." )
      			     RELACS_RTAI=no
			     USE_RTAI=no ] ) ],
	[ RELACS_RTAI=no ] )
