...
int syncSECSubdevice = 2;   /* Digital I/O on NI board */
int syncSECLine = 6;        /* Pin 6 */
long syncSECPulse = 10000;  /*! Length of the inject pulse generated by the SEC in ns. */
long syncSECTimeCor = 1500;  /*! Correction for rtai time measurement in ns. */
...
#ifdef ENABLE_SYNCSEC
  if ( subdev[iS].type == SUBDEV_IN && syncSECDevice == 0 ) {
    syncSECDevice = device[iDev].devP;
    if ( comedi_dio_config( syncSECDevice, syncSECSubdevice, syncSECLine, COMEDI_OUTPUT ) < 0 ) {
      comedi_perror( "openComediDevice() -> SYNCSEC_CONFIGURE" );
      ERROR_MSG( "openComediDevice: comedi_dio_config single channel on subdevice %d, channel %d, direction %d failed!\n",
		 syncSECSubdevice, syncSECLine, COMEDI_OUTPUT );
      return -EFAULT;
    }
  }
#endif
...
#ifdef ENABLE_SYNCSEC
    currentfac = 0.25 * ( difftime - syncSECTimeCor ) / syncSECPulse;
    /*    currentfac = 1.0;*/
#endif
