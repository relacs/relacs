...
void MultiSTA::analyze( const EventData &spiketrain, double interval )
{
  ...
  for ( unsigned int k=0; k<STAs.size(); k++ ) {
    ...
    // for each spike find corresponding snippet in the input trace (numerics/include/relacs/eventdata.h):
    const InData &intrace = trace( SpikeTrace[k] );
    ...
    for ( int i=n; i<=p; i++ ) {
      ...
      //      cerr << "SNIPPET " << snippet.rangeFront() << " " << snippet.rangeBack() << endl;
