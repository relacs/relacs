    ...
    IInFac( 1.0 )
{
  ...
  addInteger( "repeat", "Number of repetitions of the whole V-I curve measurement", 0, 0, 10000, 1 );
...
VICurve::Data::Data( double delay, double duration, double stepsize,
  ...
  : DC( 0.0 ),
    ...
    VSSsd( 0.0 ),
    SpikeCount( 0.0 ),
    MeanTrace( -delay, 2.0*duration, stepsize, 0.0F ),
    SquareTrace( -delay, 2.0*duration, stepsize, 0.0F ),
    StdevTrace( -delay, 2.0*duration, stepsize, 0.0F )
{
  if ( current )
